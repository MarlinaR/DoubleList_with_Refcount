...
FILE* gpipe = popen("gnuplot -persist", "w");
...
TEST_CASE("Threaded tests", "[threads]") {
    ...
    SECTION("Threaded pushing, correct size of list, erase after threaded pushing") {
        ...
        while (threadAmount <= neededThreadsAmount) {
            ...
            auto normalStart = std::chrono::system_clock::now();
            ThreadedTest(threadAmount, repeatEveryTest, totalPushes);
            auto normalDuration = chrono::duration_cast<chrono::milliseconds>(std::chrono::system_clock::now() - normalStart).count();
            cout << "Normal lock; " << threadAmount << " threads; " << normalDuration << " sec." << endl;

            auto spinlockStart = std::chrono::system_clock::now();
            SLThreadedTest(threadAmount, repeatEveryTest, totalPushes);
            auto spinlockDuration = chrono::duration_cast<chrono::milliseconds>(std::chrono::system_clock::now() - spinlockStart).count();
